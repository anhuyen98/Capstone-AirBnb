...
export const getUserByIdThunk = createAsyncThunk(
  "user/getUserById",
  async (path: number, { rejectWithValue }) => {
    try {
...
export const postUserThunk = createAsyncThunk(
  "user/postUser",
  async (payload: RegisterSchemaType, { rejectWithValue, dispatch }) => {
    try {
...
export const updateUserByIdThunk = createAsyncThunk(
  "user/updateUserById",
  async (
    payload: { id: number; dataPayLoad: UserLogin },
    { rejectWithValue, dispatch }
  ) => {
    try {
      const { id, dataPayLoad } = payload;
      const data = await userServices
        .updateUserById(id, dataPayLoad)
